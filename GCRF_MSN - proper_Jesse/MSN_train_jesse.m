function [alpha,beta,MSE_train_GCRF_MSN, output] = MSN_train_jesse(YMlist,Rlist,S1,S2)
...
if timeversions1 > 1 
    ...
    parfor i = 1:timeversions1
        D1 = sum(S1(i));  D1 = D1.^(-.5);
        S1_norm = S1(i) .* (D1 * D1'); 
        [V_St,D_St] = eig(S1_norm); D_St = diag(D_St);
    ...
    D1 = D1.^(-.5);
    S1_norm = S1 .* (D1' * D1);
    [V_S,D_S] = eig(S1_norm); D_S = diag(D_S);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
end     

%Write The Same Normalize Parallel Loop For S2 (perhaps write a method)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
D2 = sum(S2);  
D2 = D2.^(-.5);
S2_norm = S2 .* (D2' * D2); 
...
%Theta0 = [ 3.180927887083992, 5.264496154890667, 25.137070504694925];
...
%Run Gradient
options = optimset('Algorithm','interior-point', 'Display','Off','MaxIter',300,'GradObj','on','TolX',1e-100,'TolCon',1e-50); %optimization
...
MSE_train_GCRF_MSN = (reshape(YMlist,[n,1]) - mu)' * (reshape(YMlist,[n,1]) - mu)/n;
